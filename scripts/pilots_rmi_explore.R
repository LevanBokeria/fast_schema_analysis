...
# Estimate the learning function #############################################


fit_learning_asymptote <- function(p,t,y){
        print(p)
...
learning_asymptote <- mean_by_rep %>%
        group_by(ptp_trunk,condition) %>%
        do(as.data.frame(
                optim(c(0.5,0.1),
                      fit_learning_asymptote,
                      gr = NULL,
                      seq(1,48),
                      .$correct_rad_63_mean,
                      method = 'L-BFGS-B',
...
mean_by_rep$y_hat_learning_asymptote <- NA
...
for (iRow in seq(1,nrow(mean_by_rep))){
        ...
        row_idx1 = which(learning_asymptote$ptp_trunk == curr_ptp & 
                                learning_asymptote$condition == curr_condition)
        
        curr_c1 = learning_asymptote$c[row_idx1]
        curr_a1 = learning_asymptote$a[row_idx1]        
        ...
        for (iRep in seq(1,8)){
                ...
                mean_by_rep$y_hat_learning_asymptote[idx1] <- curr_a1*(1-exp(-curr_c1*(iRep-1)))
