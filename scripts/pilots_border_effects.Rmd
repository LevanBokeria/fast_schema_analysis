...
```{r border-effects, fig.width=15, fig.height=4, warning=FALSE, message=FALSE}
...
border_effects_by_session <-
        ...
        group_by(ptp_trunk,
                 ...
                 accuracy_type,
                 session) %>%
        summarise(by_session_mean = mean(correct_mean),
                  by_session_sd   = sd(correct_sd),
                  n = n()) %>% 
        ungroup() 
...
fig_border_effects <- mean_by_border_dist_rep_long %>%
        filter(accuracy_type %in% c('correct_one_square_away')) %>%
        filter(border_dist %in% c(1,2,3,4)) %>% 
        # filter(ptp_trunk == '604fa68...') %>%
        droplevels() %>% 
        mutate(border_dist = as.factor(border_dist)) %>%
...
sum_stats_each_participant %>%
        ...
        facet_grid(accuracy_type~condition) +
        ggtitle('Island PAs only')

sum_stats_each_participant %>%
        pivot_longer(cols = c(last_two_mean,last_four_mean),
                     names_to = 'how_many_last',
                     values_to = 'average_acc') %>%
        filter(neighbor_status == 'neighbor',
               accuracy_type %in% c('correct_exact','correct_one_square_away'),
               !condition %in% c('random_locations','no_schema')) %>%
        droplevels() %>%
        ggplot(aes(x=average_acc, group=how_many_last, color=how_many_last)) +
...
sum_stats_each_participant %>%
        ...
        pivot_wider(id_cols = c(ptp_trunk,
                    ...
                    names_from = neighbor_status,
                    values_from = c) %>% 
        mutate(neighbor_more = neighbor > island,
               neighbor_same = neighbor == island,
               neighbor_less = neighbor < island) %>%
        group_by(condition,
                 accuracy_type) %>%
        summarise(across(c(neighbor_more,
                           neighbor_same,
                           neighbor_less),sum),
                  total_n = n()) %>%
